...
​			 配置文件位置:  ~/.gitconfig
 		...
 		3. 配置当前项目： git config  [选项]     		**此配置必须在某个Git项目下执行**
...
```
e.g.  git config --global 表示只有当前用户tarena@tarena可以使用此邮箱。
tarena@tarena:~$  git config --global ... ...
```
...
```python
e.g. 将用户名设置为hfk
sudo git config --system user.name hfk   		hfk:自定义的用户名
e.g. 查看配置完成的用户名
cat /etc/gitconfig
```
...
e.g.  git config --global 表示只有当前用户tarena@tarena可以使用此邮箱。
tarena@tarena:~$  git config --global user.email 88888888@qq.com
```
...
```python
git help 命令名     '例如：git help init' ---查看init初始化仓库命令的用法。
```
