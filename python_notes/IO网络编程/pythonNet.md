...
| 查看文件内容| cat |
| 清屏 | ctrl l|
| 将字符串友好的显示出计算机识别的字节码（串） | s.encode() |
| 将友好的计算机字节码（串）转换为我们看懂的字符串 | bytes.decode() |
| 查看当前路径 | pwd |
| 回到主目录/家目录 | cd 回车键 |
| 切换目录 | cd /目录        cd 指定路径 |
| 切换到上一级目录 | cd .. |
...
所以的字符串都可以转换为字节串，并不是所有的字节串都可以转化字符串。
...
1. 读取文件  			全部读取/或者分段读取：分段读取，第二次读取在第一次读取的基础上继续读取。						

>​									
>
>文件是文本文件，则打开可以是文本方式或者二进制方式，如果文本方式打开，读写的都是以字符串方式读取或者写入内容；如果以二进制方式打开，读写都是以字节穿读取或者写入内容。
>
>文件时二进制文件，则打开方式必须二进制打开，此时文件读写内容也是字节串。
>
>read([size])	
>
>功能： 用来直接读取文件中字符。
...
1. 定义			r , w 打开文件偏移量在开头，改写从开头开始     a 打开文件偏移量在结尾，改写在结尾开始
...
>结构
>>IPv4  点分十进制表示 172.40.91.185 每部分取值范围0--255     32位
>>IPv6  128位 扩大了地址范围
...
> 三次握手（建立连接）							client:客户端	server:服务端
>>客户端向服务器发送消息报文请求连接
>>服务器收到请求后，回复报文确定可以连接
>>客户端收到回复，发送最终报文连接建立
...
参数：  socket_family  网络地址类型 AF_INET表示ipv4	AF_INET6表示ipv6
	...
	proto  通常为0  :选择子协议	TCP UDP没有子协议只有选择0
返回值： 套接字对象
```
2. 绑定地址	bind()

>本地地址 ： 'localhost' 或者 '127.0.0.1'    		 (localhost:自动搜索本机地址)
>网络地址 ： '172.40.91.185'
>自动获取地址： '0.0.0.0'
...
sockfd.bind(addr)
功能： 绑定本机网络地址
参数： 二元元组 (ip,port)  ('0.0.0.0',8888)  -------：   IP地址,端口号
```
...
```python
sockfd.connect(server_addr)     -----：客户端IP地址，端口号
功能：连接服务器
参数：（二元）元组  服务器地址
```
